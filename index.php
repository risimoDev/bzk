  ...
  <section class="relative overflow-hidden h-[560px] md:h-[680px]">
    ...
    <div class="container mx-auto px-4 py-16 md:py-28 relative z-20 flex items-center h-full">
      <div class="max-w-3xl mx-auto text-center text-white">
        <h2 class="text-lg md:text-xl text-white/90 mb-2 font-medium">Создаём яркие решения для бизнеса и частных
          клиентов</h2>
        <h1 class="text-3xl md:text-5xl font-extrabold text-white mb-4 leading-tight">BZK PRINT</h1>
        <p class="text-base md:text-lg text-white/90 mb-8">От визиток до масштабных проектов - качество, которое видно и
          ощущается. Мы делаем так, чтобы клиенты возвращались снова.</p>

        <div class="flex flex-col sm:flex-row gap-4 justify-center">
          <a href="/catalog"
            class="inline-flex items-center justify-center px-8 py-4 bg-white text-[#118568] rounded-xl hover:scale-105 transition transform font-semibold shadow-lg">
            Перейти в каталог
          </a>
          <a href="/contacts"
            class="inline-flex items-center justify-center px-8 py-4 bg-transparent border-2 border-white text-white rounded-xl hover:bg-white hover:text-[#118568] transition transform font-semibold">
            Связаться с нами
          </a>
        </div>
      </div>
    ...
    <div
      class="absolute -bottom-12 right-8 opacity-30 animate-slow-rotate w-44 h-44 rounded-full bg-white/5 pointer-events-none z-30">
    </div>
  </section>
    ...
    <div class="container mx-auto px-4 max-w-7xl">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center mb-10">
        <div class="lg:col-span-2">
          ...
          <p class="text-gray-600 mb-6">Простая и прозрачная схема от заказа до готовности. Подключим дизайнеров и
            менеджеров под ваш проект.</p>
      ...
      <div class="mb-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="bg-white rounded-2xl p-6 shadow-md hover:shadow-xl transform transition hover:-translate-y-2"
            data-animate>
            ...
            <p class="text-gray-600 text-sm">Мы используем профессиональное оборудование для стабильного результата и
              контроля качества.</p>
          </div>
          ...
          <div class="bg-white rounded-2xl p-6 shadow-md hover:shadow-xl transform transition hover:-translate-y-2"
            ...
            <p class="text-gray-600 text-sm">Каждый проект уникален: подбираем материалы, тираж и технологии под Вашу
              задачу и бюджет.</p>
          </div>
          ...
          <div class="bg-white rounded-2xl p-6 shadow-md hover:shadow-xl transform transition hover:-translate-y-2"
            ...
            <p class="text-gray-600 text-sm">На странице заказа вы можете отслеживать текущий этап производства, а также
              задать вопросы или внести изменения - просто напишите в онлайн-чат.</p>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="py-12 bg-gradient-to-b from-white to-[#DEE5E5]">
    <div class="container mx-auto px-4 max-w-7xl">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        ...
        <a href="/catalog" class="text-sm text-[#118568] font-medium self-start md:self-auto">Смотреть весь каталог
          →</a>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <?php foreach ($popularProducts as $product):
          ...
          <a href="/service?id=<?php echo $product['id']; ?>"
            class="group block bg-white rounded-2xl shadow-md overflow-hidden transform transition-all duration-300 hover:-translate-y-2"
            data-animate>
            ...
            <div class="p-4">
              <h4 class="font-semibold text-gray-800 mb-1 line-clamp-2"><?php echo htmlspecialchars($product['name']); ?>
              </h4>
              <p class="text-sm text-gray-600 line-clamp-2 mb-3"><?php echo htmlspecialchars($product['description']); ?>
              </p>
    ...
    <div class="container mx-auto px-4 max-w-7xl">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        ...
        <button id="open-review-modal"
          class="px-4 py-2 bg-[#118568] text-white rounded-lg hover:bg-[#0f755a] transition self-start md:self-auto">Оставить
          отзыв</button>
      </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 items-center">
        <?php if (!empty($partners)): ?>
          <?php foreach ($partners as $p): ?>
            <div
              class="flex items-center justify-center p-4 bg-white rounded-2xl shadow-sm hover:shadow-md transform transition hover:scale-105">
              <img src="<?php echo htmlspecialchars($p['logo_url']); ?>" alt="<?php echo htmlspecialchars($p['name']); ?>"
                class="max-h-12 object-contain" loading="lazy">
            </div>
          <?php endforeach; ?>
        <?php else: ?>
          <?php for ($i = 0; $i < 6; $i++): ?>
  ...
  <section class="py-12 bg-gradient-to-r from-[#118568] to-[#17B890]">
    <div class="container mx-auto px-4 max-w-4xl text-center">
      ...
      <div class="flex flex-col sm:flex-row gap-4 justify-center">
        ...
        <a href="/catalog"
          class="px-6 py-3 bg-transparent border border-white text-white rounded-lg font-semibold">Каталог</a>
      </div>
    </div>
  </section>
  ...
  <div class="bg-white rounded-2xl w-full max-w-2xl mx-4 p-6 relative">
    ...
    <?php if (!isset($_SESSION['user_id'])): ?>
      <div id="login-required" class="p-6 bg-yellow-50 rounded-lg">
        <p class="mb-4">Чтобы оставить отзыв, пожалуйста, <a href="/login" class="text-[#118568] underline">войдите в
            аккаунт</a>.</p>
      </div>
    <?php else: ?>
      <form id="review-form" class="space-y-4">
        ...
        <div>
          ...
          <input type="text" name="name" class="mt-1 block w-full border rounded-lg px-3 py-2"
...
<style>
  ...
  #dot-grid-canvas {
    ...
    /* Убедитесь, что это закомментировано или удалено */
    z-index: 10;
    /* Убедитесь, что z-index указан в HTML, если еще не сделали */
  }
</style>
