...
function handleConnectCommand($chat_id, $text, $first_name)
{
    ...
    if (count($parts) < 2) {
        ...
        $message .= "Используйте: /connect your@email.com\n";
        $message .= "Например: /connect user@example.com";
        sendTelegramMessage($chat_id, $message);
        return;
    }
    ...
    try {
        ...
        if ($stmt->execute([$chat_id, $user['id']])) {
            ...
            $message .= "📦 О новых заказах\n";
            $message .= "📋 О назначенных задачах\n";
