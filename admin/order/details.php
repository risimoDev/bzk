...
// Обработка: дата готовности (установка/продление/редактирование)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_ready'])) {
  $token = $_POST['csrf_token'] ?? '';
  if (!isset($_SESSION['csrf_token']) || !is_string($token) || !hash_equals($_SESSION['csrf_token'], $token)) {
    $_SESSION['notifications'][] = ['type' => 'error', 'message' => 'Ошибка безопасности (CSRF).'];
    header("Location: " . $_SERVER['REQUEST_URI']);
  ...
  $mode = $_POST['ready_mode'] ?? 'set'; // set|edit|prolong
  $reason = trim($_POST['ready_reason'] ?? '');
  $prev = $order['ready_date'] ?? null;
  $newDate = null;
  try {
    if ($mode === 'prolong') {
      ...
      if ($input === '')
        throw new Exception('Укажите дату готовности.');
      $dt = new DateTime($input);
      $newDate = $dt->format('Y-m-d H:i:s');
    }
    $pdo->beginTransaction();
    $pdo->prepare("UPDATE orders SET ready_date = ? WHERE id = ?")->execute([$newDate, $order_id]);
    $order['ready_date'] = $newDate;
    $change_type = $mode === 'prolong' ? 'prolong' : (!empty($prev) ? 'edit' : 'set');
    $ins = $pdo->prepare("INSERT INTO order_ready_history (order_id, previous_date, new_date, change_type, reason, changed_by) VALUES (?, ?, ?, ?, ?, ?)");
    $ins->execute([$order_id, $prev, $newDate, $change_type, $reason !== '' ? $reason : null, $_SESSION['user_id'] ?? null]);
    $pdo->commit();
    $_SESSION['notifications'][] = ['type' => 'success', 'message' => 'Дата готовности обновлена.'];
  } catch (Exception $e) {
  ...
  try {
    ...
    ");
      ...
      if ((int) $row['is_custom'] === 1) {
        ...
        $unit_price = round((float) ($_POST['unit_price'] ?? 0), 2);
        if ($item_name === '' || $unit_price <= 0)
          throw new Exception('Укажите корректные имя и цену.');
  ...
  <div class="container mx-auto px-4 max-w-7xl">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
      ...
      <div class="w-full md:w-auto flex gap-2">
        <?php echo backButton(); ?>
        <a href="/admin/orders"
          class="px-4 py-2 bg-[#118568] text-white rounded-lg hover:bg-[#0f755a] transition-colors duration-300 text-sm">Все
          заказы</a>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div class="lg:col-span-2 space-y-8">
          ...
          <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-6">
            <div>
              ...
              <div class="flex flex-wrap items-center gap-2">
                <span
                  class="px-3 py-1 rounded-full text-sm font-medium <?php echo $status_colors[$order['status']] ?? 'bg-gray-100 text-gray-800'; ?>">
                  <?php echo htmlspecialchars($statuses[$order['status']] ?? 'Неизвестно'); ?>
                </span>
                <?php if ($is_urgent): ?>
                  <span class="px-3 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800">Срочный заказ
                    (+50%)</span>
                <?php endif; ?>
                <span class="ml-1 text-gray-600">Создан:
            ...
            <form action="" method="POST" class="flex items-center gap-2">
              ...
              <select name="update_status"
                class="rounded-l-lg border-gray-300 focus:border-[#118568] focus:ring-[#17B890]">
              ...
              <button type="submit" class="px-4 py-2 bg-[#118568] text-white rounded-r-lg hover:bg-[#0f755a]"
            ...
            <div class="bg-gray-50 rounded-2xl p-4">
              ...
              <div class="space-y-2">
                <p class="text-gray-700"><span class="font-medium">Имя:</span>
                  <?php echo htmlspecialchars($order['user_name'] ?? 'Не указано'); ?></p>
                <p class="text-gray-700"><span class="font-medium">Email:</span>
                  <?php echo htmlspecialchars($order['email'] ?? 'Не указано'); ?></p>
                <p class="text-gray-700"><span class="font-medium">Телефон:</span>
                  <?php echo htmlspecialchars($order['phone'] ?? 'Не указано'); ?></p>
            ...
            <div class="bg-gray-50 rounded-2xl p-4">
              <h3 class="font-bold text-gray-800 mb-3 flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-[#118568]" fill="none"
                  viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0L5 15.243V19a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h14a2 2 0 012 2v14a2 2 0 01-2 2h-5z" />
                </svg>
                Доставка
              </h3>
              <div class="space-y-2">
                <p class="text-gray-700"><span class="font-medium">Адрес:</span>
                  <?php echo htmlspecialchars($order['shipping_address'] ?? 'Не указано'); ?></p>
                <p class="text-gray-700"><span class="font-medium">Контакт:</span>
            ...
            <div class="md:col-span-2 bg-gray-50 rounded-2xl p-4">
              <h3 class="font-bold text-gray-800 mb-3 flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-[#118568]" fill="none"
                  viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0L5 15.243V19a2 2 0 01-2 2H3a2 2 0 01-2-2V5a2 2 0 012-2h14a2 2 0 012 2v12a2 2 0 01-2 2h-3.586l-4.243-4.243z" />
                </svg>
              ...
              <div class="flex flex-col md:flex-row gap-4 md:items-end">
                <form method="POST" class="flex items-center gap-2">
                  ...
                  <select name="payment_status" id="payment_status" onchange="this.form.submit()"
                    ...
                    <option value="unpaid" <?php echo ($accounting['status'] ?? '') === 'unpaid' ? 'selected' : ''; ?>>Не
                      оплачен</option>
                    <option value="partial" <?php echo ($accounting['status'] ?? '') === 'partial' ? 'selected' : ''; ?>>
                      Частично</option>
                    <option value="paid" <?php echo ($accounting['status'] ?? '') === 'paid' ? 'selected' : ''; ?>>Оплачен
                    </option>
                  </select>
                </form>
            ...
            <div class="md:col-span-2 bg-gray-50 rounded-2xl p-4">
              <h3 class="font-bold text-gray-800 mb-3 flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-[#118568]" fill="none"
                  viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2h-3.17a4 4 0 01-7.66 0H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                Дата готовности
              </h3>
              <div class="space-y-3">
                <p class="text-gray-700">
                  Текущая дата: <span
                    class="font-medium"><?php echo $order['ready_date'] ? date('d.m.Y H:i', strtotime($order['ready_date'])) : 'не установлена'; ?></span>
                </p>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <form method="POST" class="bg-white p-3 rounded-xl border">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrf_token); ?>">
                    <input type="hidden" name="update_ready" value="1">
                    <input type="hidden" name="ready_mode" value="edit">
                    <label class="block text-xs font-medium text-gray-700 mb-1">Установить/изменить дату</label>
                    <input type="datetime-local" name="ready_date" class="w-full border rounded px-2 py-2 mb-2"
                      value="<?php echo $order['ready_date'] ? date('Y-m-d\TH:i', strtotime($order['ready_date'])) : ''; ?>">
                  ...
                  <form method="POST" class="bg-white p-3 rounded-xl border">
                    ...
                    <label class="block text-xs font-medium text-gray-700 mb-1">Продлить на (дней)</label>
                    <input type="number" min="1" name="prolong_days" class="w-full border rounded px-2 py-2 mb-2"
                      placeholder="Напр., 3">
                    <input type="text" name="ready_reason" class="w-full border rounded px-2 py-2 mb-2"
                      placeholder="Причина продления (необязательно)">
                    <button type="submit"
                ...
                <?php
                // История
                $hist = $pdo->prepare("SELECT * FROM order_ready_history WHERE order_id = ? ORDER BY id DESC LIMIT 5");
                $hist->execute([$order_id]);
        ...
        <div class="bg-white rounded-3xl shadow-2xl p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <button type="button" id="toggle-add-panel"
              class="px-3 py-2 text-sm rounded bg-[#118568] text-white hover:bg-[#0f755a]">
              Добавить позицию
            </button>
          </div>
          ...
          <div id="add-panel" class="hidden bg-white rounded-2xl p-4 mb-6 border border-gray-200">
            <div class="flex gap-2 mb-4">
              <button type="button" data-tab="tab-catalog"
                class="add-tab px-3 py-2 rounded bg-[#118568] text-white text-sm">Из каталога</button>
              <button type="button" data-tab="tab-custom"
                class="add-tab px-3 py-2 rounded bg-gray-200 text-gray-800 text-sm">Своя позиция</button>
            </div>
            ...
            <div id="tab-catalog" class="tab-content">
              <form method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="md:col-span-2">
                  ...
                  <input type="text" id="prod-search" placeholder="Поиск..."
              ...
              <form method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="md:col-span-2">
                  ...
                  <input type="text" name="item_name" required class="w-full border rounded px-3 py-2"
                    placeholder="Например, Доработка макета">
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">Количество</label>
                ...
                <div class="md:col-span-3">
                  ...
                  <textarea name="item_note" class="w-full border rounded px-3 py-2" rows="2"
                    placeholder="Комментарии для производства/клиента"></textarea>
                </div>
                ...
                <div class="md:col-span-3 mt-2">
                  ...
                  <div id="expense-fields"
                    class="hidden mt-3 grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-lg border">
                    <div>
                      <label class="block text-sm font-medium text-gray-700 mb-1">Сумма расхода, ₽</label>
                      <input type="number" step="0.01" min="0.01" name="exp_amount"
                        class="w-full border rounded px-3 py-2" placeholder="Например, 500.00">
                    </div>
                    <div>
                      ...
                      <select name="exp_category_id" class="w-full border rounded px-3 py-2">
                        ...
                        <?php foreach ($expense_categories as $cat): ?>
                          <option value="<?php echo (int) $cat['id']; ?>"><?php echo htmlspecialchars($cat['name']); ?>
                          </option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                    <div class="md:col-span-3">
                      ...
                      <input type="text" name="exp_desc" id="exp_desc" class="w-full border rounded px-3 py-2"
                        placeholder="Например, Внеплановая допечатная подготовка">
            ...
            <?php else: ?>
              <?php foreach ($order_items as $item):
                ...
                <div class="p-4 bg-gray-50 rounded-2xl border hover:bg-gray-100 transition">
                  ...
                  <div class="mt-2 text-sm text-gray-700">
                    Количество: <?php echo (int) $item['quantity']; ?> | Цена за единицу:
                  ...
                  <div class="mt-3 flex flex-col md:flex-row gap-2">
                    ...
                    <form method="POST" class="flex flex-wrap items-end gap-2 bg-white p-3 rounded-lg border">
                      ...
                      <?php if ($is_custom): ?>
                        ...
                        <div class="flex-1 min-w-[200px]">
                          ...
                          <input type="text" name="item_note"
                            value="<?php echo htmlspecialchars($item['item_note'] ?? ''); ?>"
                      ...
                      <button type="submit"
                        class="px-3 py-2 bg-[#17B890] text-white text-sm rounded hover:bg-[#14a380]">Сохранить</button>
                    </form>
                    ...
                    <form method="POST" onsubmit="return confirm('Удалить позицию?');" class="self-start">
                      ...
                      <input type="hidden" name="item_id" value="<?php echo (int) $item['item_id']; ?>">
                      <button type="submit"
                        class="px-3 py-2 bg-red-600 text-white text-sm rounded hover:bg-red-700">Удалить</button>
                    </form>
                  </div>
                </div>
              <?php endforeach; ?>
      ...
      <div class="space-y-8">
        <div class="bg-white rounded-3xl shadow-2xl p-6">
          ...
          <div class="space-y-4">
            <div class="flex justify-between"><span class="text-gray-600">Товарных позиций:</span><span
                class="font-medium"><?php echo count($order_items); ?></span></div>
            <div class="flex justify-between"><span class="text-gray-600">Сумма по строкам:</span><span
                class="font-medium"><?php echo number_format($original_total_price, 2, '.', ' '); ?> ₽</span></div>
            <div class="flex justify-between"><span class="text-gray-600">Доставка:</span><span
                class="font-medium">Бесплатно</span></div>
            ...
            <?php if ($chat): ?>
              <form action="" method="POST" class="space-y-4">
                ...
                <button type="submit" name="assign_user"
                  class="w-full px-4 py-2 bg-[#118568] text-white rounded-lg hover:bg-[#0f755a]">Назначить</button>
              </form>
              <?php if (!empty($chat['assigned_user_name'])): ?>
                <div class="mt-4 p-3 bg-[#DEE5E5] rounded-lg text-sm">
                  ...
                  <?php if ($chat['assigned_user_role'] === 'admin'): ?><span
                      class="px-1 py-0.5 bg-red-100 text-red-800 text-xs rounded ml-1">(Админ)</span><?php endif; ?>
                </div>
              <?php endif; ?>
            <?php else: ?>
              ...
              <?php else: ?>
                <?php foreach ($messages as $message):
                  ...
                  <div class="mb-4 <?php echo $is_own_message ? 'text-right' : 'text-left'; ?>">
                    ...
                    <?php endif; ?>
                    <div
                      class="inline-block max-w-xs md:max-w-md px-4 py-3 rounded-2xl <?php echo $is_own_message ? ' bg-[#118568] text-white rounded-tr-none' : ' bg-gray-200 text-gray-800 rounded-tl-none'; ?>">
              ...
              <?php if ($chat): ?>
                <form id="admin-chat-form" class="flex gap-2">
                  ...
                  <button type="submit" id="admin-send-button"
                    class="px-4 py-2 bg-[#118568] text-white rounded-lg hover:bg-[#0f755a] flex items-center justify-center">
                    <svg id="admin-send-icon" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                      viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
