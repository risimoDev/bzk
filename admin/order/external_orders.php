  ...
  <div class="container mx-auto px-6 max-w-7xl">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
      ...
      <div class="flex gap-3">
        <?php echo backButton(); ?>
        <a href="/admin/order/add_external.php" class="px-5 py-2.5 bg-[#118568] text-white rounded-xl hover:bg-[#0f755a] transition text-sm font-medium">
          + Новый заказ
        </a>
        <a href="/admin/buhgalt/" class="px-5 py-2.5 bg-[#17B890] text-white rounded-xl hover:bg-[#15a081] transition text-sm font-medium">
          Бухгалтерия
        </a>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
      <div class="bg-white rounded-2xl shadow-lg p-6 text-center">
        ...
        <p class="text-3xl font-bold text-[#118568] mt-2"><?php echo number_format($stats['total_orders']); ?></p>
      </div>
      <div class="bg-white rounded-2xl shadow-lg p-6 text-center">
        <h3 class="text-sm font-medium text-gray-600">Общая выручка</h3>
        <p class="text-3xl font-bold text-[#118568] mt-2"><?php echo number_format($stats['total_revenue'], 0, '', ' '); ?> ₽</p>
      </div>
      <div class="bg-white rounded-2xl shadow-lg p-6 text-center">
        <h3 class="text-sm font-medium text-gray-600">Оплачено</h3>
        <p class="text-3xl font-bold text-green-600 mt-2"><?php echo number_format($stats['paid_revenue'], 0, '', ' '); ?> ₽</p>
      </div>
      <div class="bg-white rounded-2xl shadow-lg p-6 text-center">
        <h3 class="text-sm font-medium text-gray-600">Не оплачено</h3>
        <p class="text-3xl font-bold text-red-600 mt-2"><?php echo number_format($stats['unpaid_revenue'], 0, '', ' '); ?> ₽</p>
      </div>
    </div>
      ...
      <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <select name="status" class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-[#118568] focus:ring-2 focus:ring-[#9DC5BB]">
            ...
            <option value="unpaid" <?php echo $status_filter === 'unpaid' ? 'selected' : ''; ?>>Не оплачен</option>
        ...
        <div class="flex items-end">
          <a href="?" class="w-full py-3 bg-gray-200 text-gray-700 text-center rounded-lg hover:bg-gray-300 transition font-medium">
            Сбросить
          </a>
        </div>
      </form>
    ...
    <?php if (empty($orders)): ?>
      <div class="text-center py-16">
        <div class="bg-white rounded-3xl shadow-lg p-12">
          ...
          <a href="/admin/order/add_external.php" class="inline-block px-8 py-4 bg-[#118568] text-white rounded-xl hover:bg-[#0f755a] transition font-medium">
            + Создать заказ
          </a>
        </div>
      </div>
        ...
        <?php foreach ($orders as $order): ?>
          <div class="bg-white rounded-2xl shadow-lg hover:shadow-xl transition p-6">
            ...
            <div class="flex justify-between items-start mb-4">
              <div>
                <h3 class="text-lg font-bold text-gray-800"><?php echo htmlspecialchars($order['client_name']); ?></h3>
                <p class="text-sm text-gray-600">#<?php echo $order['id']; ?> • <?php echo date('d.m.Y H:i', strtotime($order['created_at'])); ?></p>
              </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4 mb-4">
              <div class="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  ...
                  <span class="font-medium"><?php echo number_format($order['tax_amount'] ?: 0, 0, '', ' '); ?> ₽</span>
                </div>
                <div>
