...
$text_attributes = array_filter($attributes, function ($attr) {
  return $attr['type'] === 'text'; });
$choice_attributes = array_filter($attributes, function ($attr) {
  return $attr['type'] !== 'text'; });
?>
      ...
      <div class="bg-white rounded-3xl shadow-2xl p-6">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
          ...
          <div class="text-gray-600">
            Найдено: <?php echo count($attributes); ?>
            <?php echo count($attributes) == 1 ? 'характеристика' : (count($attributes) < 5 ? 'характеристики' : 'характеристик'); ?>
          </div>
        </div>
              ...
              <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                        ...
                        foreach ($values as $value): ?>
                          <span class="px-3 py-1 bg-white border border-[#17B890] text-[#17B890] text-sm rounded-full">
                            ...
                            <?php if ($value['price_modifier'] != 0): ?>
                              <span class="text-xs">
                                (<?php echo $value['price_modifier'] > 0 ? '+' : ''; ?><?php echo number_format($value['price_modifier'], 2, '.', ''); ?>
                                руб.)
                              </span>
                            <?php endif; ?>
                          </span>
                ...
                <div class="flex gap-2">
                  ...
                  <form action="" method="POST"
                    onsubmit="return confirm('Вы уверены, что хотите удалить эту характеристику?')" class="m-0">
                    <?php echo csrf_field(); ?>
