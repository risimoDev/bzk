...
try {
    // Загружаем получателей для текущей итерации
    $stmt = $pdo->prepare("
    ...
    foreach ($pending_recipients as $recipient) {
        ...
        // === EMAIL ===
        if ($recipient['email_status'] === 'pending' && filter_var($user_email, FILTER_VALIDATE_EMAIL)) {
            try {
                $mail->clearAddresses();
                ...
                <html><body style='font-family:Arial,sans-serif;line-height:1.6;color:#333;'>
                    <div style='max-width:600px;margin:auto;padding:20px;'>
                        <div style='background:linear-gradient(135deg,#118568,#17B890);color:white;padding:20px;border-radius:10px 10px 0 0;'>
                ...
                if ($mail->send()) {
                    $stmt = $pdo->prepare("
                        UPDATE mass_message_recipients 
                ...
                $stmt = $pdo->prepare("
                    ...
                    WHERE id = ?
                ");
                $stmt->execute([$e->getMessage(), $recipient_id]);
        ...
        if ($recipient['telegram_status'] === 'pending' && !empty($telegram_chat_id)) {
            try {
                ...
                if ($telegram->sendMessage($telegram_chat_id, $telegram_message)) {
                    $stmt = $pdo->prepare("
                        UPDATE mass_message_recipients 
                        SET telegram_status = 'sent', telegram_sent_at = NOW() 
                        WHERE id = ?
                    ");
                    $stmt->execute([$recipient_id]);
                    $telegrams_sent++;
...
<main class="min-h-screen bg-gradient-to-br from-[#DEE5E5] to-[#9DC5BB] py-10">
    <div class="container mx-auto px-6 max-w-4xl">
        <div class="text-center mb-8">
            ...
            <p class="text-lg text-gray-700"><?php echo htmlspecialchars($message['title'] ?? 'Неизвестная рассылка'); ?></p>
