  ...
  <div class="container mx-auto max-w-3xl bg-white p-6 rounded-2xl shadow-lg">
    ...
    <form method="post" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="name" value="<?= htmlspecialchars($material['name']); ?>"
          class="w-full border rounded px-3 py-2" required>
      </div>
      <div>
        <label class="block mb-1">Единица *</label>
        <input type="text" name="unit" value="<?= htmlspecialchars($material['unit']); ?>"
          class="w-full border rounded px-3 py-2" required>
      </div>
      <div>
        <label class="block mb-1">Описание</label>
        <textarea name="description"
          class="w-full border rounded px-3 py-2"><?= htmlspecialchars($material['description']); ?></textarea>
      </div>
