  ...
  <div class="container mx-auto px-4 max-w-7xl">
    ...
    <div class="bg-white rounded-3xl shadow-2xl p-8 max-w-4xl mx-auto">
      <div class="flex flex-col md:flex-row md:items-start md:justify-between mb-8 pb-6 border-b border-gray-200">
        ...
        <span class="px-3 py-1 rounded-full text-sm font-medium mt-4 md:mt-0 <?php echo $supplier['is_active'] ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'; ?>">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <!-- Основная информация -->
        <div>
          <h3 class="text-lg font-bold text-gray-800 mb-4 pb-2 border-b border-gray-100">Основная информация</h3>
          ...
          <div class="space-y-4">
            ...
            <?php if (!empty($supplier['email'])): ?>
              <div class="flex">
                ...
                <div class="flex-1">
                  <a href="mailto:<?php echo e($supplier['email']); ?>" class="text-[#118568] hover:underline">
            ...
            <?php if (!empty($supplier['website'])): ?>
              <div class="flex">
                ...
                <div class="flex-1">
                  <a href="<?php echo e($supplier['website']); ?>" target="_blank" class="text-[#118568] hover:underline">
            ...
            <?php if (!empty($supplier['service_cost'])): ?>
              <div class="flex">
                ...
                <div class="flex-1 font-bold text-[#118568]"><?php echo number_format($supplier['service_cost'], 2, '.', ' '); ?> руб.</div>
              </div>
            <?php endif; ?>
          </div>
        </div>
        ...
        <div>
          <h3 class="text-lg font-bold text-gray-800 mb-4 pb-2 border-b border-gray-100">Условия сотрудничества</h3>
      ...
      <!-- Кнопки действий -->
      <div class="mt-8 pt-6 border-t border-gray-200 flex flex-col sm:flex-row sm:justify-end space-y-4 sm:space-y-0 sm:space-x-4">
        <a href="/admin/suppliers" class="px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors duration-300 font-medium text-center">
          Назад к списку
        </a>
        <a href="/admin/suppliers/edit?id=<?php echo $supplier['id']; ?>" class="px-6 py-3 bg-[#118568] text-white rounded-lg hover:bg-[#0f755a] transition-colors duration-300 font-medium text-center">
          Редактировать
        </a>
        
        <!-- Кнопка удаления с подтверждением -->
        <form method="POST" class="inline" onsubmit="return confirm('Вы уверены, что хотите удалить этого поставщика? Это действие нельзя отменить.')">
          <?php echo csrf_field(); ?>
          <input type="hidden" name="delete_supplier" value="1">
          <button type="submit" class="w-full sm:w-auto px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors duration-300 font-medium text-center">
            Удалить поставщика
          </button>
        </form>
      </div>
