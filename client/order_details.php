    ...
    if (empty($r['is_custom']) && !empty($r['attributes'])) {
        ...
        if (is_array($decoded)) {
            foreach ($decoded as $attr_id => $value_id) {
                $attr_id = (int)$attr_id; $value_id = (int)$value_id;
                if ($value_id > 0) {
...
if (!$chat) {
    ...
    if ($created_chat_id) $chat = get_chat_by_order_id($pdo, $order_id);
}
$messages = [];
$messages_total = 0;
$messages_limit = 50;
              ...
              <div class="flex flex-col sm:flex-row gap-4 p-4 bg-gray-50 rounded-2xl hover:bg-gray-100 transition">
                <div class="flex-grow">
                  ...
                  <?php if ($item['is_custom'] && !empty($item['item_note'])): ?>
                    <div class="text-sm text-gray-600 mb-2">Примечание: <?php echo nl2br(htmlspecialchars($item['item_note'])); ?></div>
                  <?php endif; ?>

                  <div class="flex flex-wrap items-center gap-4 text-sm">
                    <span class="font-medium">Количество: <?php echo (int)$item['quantity']; ?> <?php echo htmlspecialchars($unit_label); ?></span>
                    <span class="font-medium">Цена за единицу: <?php echo number_format($unit_price, 2, '.', ' '); ?> ₽</span>
                  </div>
                </div>
                <div class="flex items-center sm:items-end sm:flex-col sm:justify-end">
                  <div class="text-lg font-bold text-[#118568]"><?php echo number_format($item['price'], 2, '.', ' '); ?> ₽</div>
                </div>
              </div>
              <?php endforeach; ?>
            <?php endif; ?>
        ...
        <div class="bg-white rounded-3xl shadow-2xl flex flex-col">
          <div class="p-6 border-b border-[#DEE5E5]">
            ...
            <?php if ($chat && !empty($chat['assigned_user_name'])): ?>
              <div class="text-sm text-gray-600">Ваш менеджер: <span class="ml-1 font-medium text-[#118568]"><?php echo htmlspecialchars($chat['assigned_user_name']); ?></span></div>
            <?php else: ?>
          ...
          <div class="p-4 border-t border-[#DEE5E5]">
            <form id="chat-form" class="flex gap-2" autocomplete="off">
              ...
              <input type="text" id="message-text" name="message_text" class="flex-grow px-4 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-[#118568] focus:ring-2 focus:ring-[#9DC5BB] text-sm" placeholder="Введите сообщение..." required maxlength="1000">
              <button type="submit" id="send-button" class="px-4 py-2 bg-[#118568] text-white rounded-lg hover:bg-[#0f755a] transition-colors duration-300">Отправить</button>
            </form>
